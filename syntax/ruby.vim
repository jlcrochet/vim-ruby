...
" Delimiters {{{2
syn match rubyDelimiter /\%#=1(/ nextgroup=@rubyArguments skipwhite skipempty
syn match rubyDelimiter /\%#=1)/ nextgroup=@rubyPostfix skipwhite

syn match rubyDelimiter /\%#=1\[/ nextgroup=@rubyArguments skipwhite skipempty
syn match rubyDelimiter /\%#=1]/ nextgroup=@rubyPostfix skipwhite

syn match rubyDelimiter /\%#=1{/ nextgroup=rubyBlockParameters,@rubyArguments skipwhite skipempty
syn match rubyDelimiter /\%#=1}/ nextgroup=@rubyPostfix skipwhite
...
syn match rubyVariableOrMethod /\%#=1[^\x00-\x5E`\x7B-\x7F][^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*[?!]\=/ nextgroup=@rubyPostfix,@rubyArguments skipwhite
...
syn match rubyHashKey /\%#=1\u[^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*[?!]\=\ze::\@!/ contained nextgroup=rubyHashKeyDelimiter
syn match rubyHashKeyDelimiter /\%#=1:/ contained nextgroup=@rubyArguments skipwhite skipempty
...
" Generated by tools/syntax.vim:
syn region rubyHeredocSkip matchgroup=rubyHeredocStart start=/\%#=1<<[-~]\=\%(\(["`']\).\{-}\1\|[^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]\+\)/ end=/\%#=1\ze<<[-~]\=[^\x00-\x21\x23-\x26\x28-\x40\x5B-\x5E\x7B-\x7F]/ contains=@rubyPostfix,@rubyArguments oneline nextgroup=rubyHeredoc,rubyHeredocSkip
