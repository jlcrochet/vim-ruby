...
syn match rubyHashKeyDelimiter /\%#=1:/ contained nextgroup=@rubyRHS skipwhite skipempty

syn cluster rubyRHS contains=
      \ rubyNumber,rubyString,rubyStringArray,rubyCharacter,rubySymbol,rubySymbolArray,rubyRegex,rubyCommand,
      \ rubyHeredoc,rubyHeredocSkip,rubySelf,rubyBoolean,rubyNil,
      \ rubyKeyword,rubyDefine,rubyBlock,rubyBlockSkip,rubyDefineBlock,rubyVariableOrMethod
...
syn match rubyOperatorMethod /\%#=1\%([&|^/%]\|=\%(==\=\|\~\)\|>[=>]\=\|<\%(<\|=>\=\)\=\|[+\-~]@\=\|\*\*\=\|\[]=\=\|![@=~]\=\)/ contained nextgroup=@rubyPostfix,@rubyRHS,rubyHashKey skipwhite
...
" Delimiters {{{2
syn match rubyDelimiter /\%#=1(/ nextgroup=@rubyRHS,rubyHashKey skipwhite skipempty
syn match rubyDelimiter /\%#=1)/ nextgroup=@rubyPostfix skipwhite

syn match rubyDelimiter /\%#=1\[/ nextgroup=@rubyRHS skipwhite skipempty
syn match rubyDelimiter /\%#=1]/ nextgroup=@rubyPostfix skipwhite

syn match rubyDelimiter /\%#=1{/ nextgroup=rubyBlockParameters,@rubyRHS,rubyHashKey skipwhite skipempty
syn match rubyDelimiter /\%#=1}/ nextgroup=@rubyPostfix skipwhite

syn match rubyComma /\%#=1,/ contained nextgroup=@rubyRHS,rubyHashKey skipwhite skipempty
...
syn match rubyConstant /\%#=1\u[^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*\%(::\@!\)\@!/ nextgroup=@rubyPostfix skipwhite
syn match rubyVariableOrMethod /\%#=1[^\x00-\x5E`\x7B-\x7F][^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*[?!]\=:\@!\>/ nextgroup=@rubyPostfix,@rubyRHS,rubyHashKey skipwhite
...
syn keyword rubySuper super nextgroup=@rubyPostfix,@rubyRHS,rubyHashKey skipwhite
...
" Generated by tools/syntax.vim:
syn region rubyHeredocSkip matchgroup=rubyHeredocStart start=/\%#=1<<[-~]\=\%(\(["`']\).\{-}\1\|[^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]\+\)/ end=/\%#=1\ze<<[-~]\=[^\x00-\x21\x23-\x26\x28-\x40\x5B-\x5E\x7B-\x7F]/ contains=@rubyPostfix,@rubyRHS oneline nextgroup=rubyHeredoc,rubyHeredocSkip
...
" Generated by tools/syntax.vim:
syn match rubyMethodDefinition /\%#=1\%(\%(\u[^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*\|[^\x00-\x5E`\x7B-\x7F][^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*\)\.\)\=\%([^\x00-\x5E`\x7B-\x7F][^\x00-\x2F\x3A-\x40\x5B-\x5E`\x7B-\x7F]*[?!=]\=\|\%([&|^/%]\|=\%(==\=\|\~\)\|>[=>]\=\|<\%(<\|=>\=\)\=\|[+\-~]@\=\|\*\*\=\|\[]=\=\|![@=~]\=\)\)/ contained contains=@rubyMethodReceivers nextgroup=rubyMethodParameters,rubyMethodAssignmentOperator,rubyHashKey skipwhite
